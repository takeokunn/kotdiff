...
KOT の `NIGHT_OVERTIME_WORK_MINUTE` 列に記録された値をそのまま使用する（ローカル計算は行わない）。稼働日かどうかに関係なく実績ありの全日を集計する。

```
深夜残業合計 = Σ NIGHT_OVERTIME_WORK_MINUTE（実績ありの全日）
```
...
区間の種別は `work`（勤務）と `break`（休憩）の 2 種類。日またぎシフトは `end ≤ start` の場合にスキップする。
